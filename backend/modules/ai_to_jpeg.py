...
def process_single_file(input_path, processing_dir):
    output_path = os.path.join(processing_dir, f"{os.path.splitext(os.path.basename(input_path))[0]}.jpeg")
    try:
    ...
    for root, dirs, files in os.walk(extract_dir):
        for file in files:
            if file.endswith('.ai'):
                ...
                output_file = os.path.join(output_dir, f"{os.path.splitext(file)[0]}.jpeg")
                try:
                    convert_to_jpeg(input_file, output_file)
                except Exception as e:
                    return jsonify({"error": str(e)}), 500
